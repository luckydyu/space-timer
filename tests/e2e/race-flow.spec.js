...
test('starts a one-lap race, records the lap, and shows results', async ({ page }) => {
  ...
  await expect(page.locator('#running-lap-indicator')).toContainText('MISSION 1 / 1');
  await expect(page.locator('#running-lap-indicator')).toContainText('F-1 1');
  ...
  await expect.poll(async () => page.evaluate(() => {
    const plan = JSON.parse(localStorage.getItem('spaceTimerKitanPlan'));
...
test('undoes the latest completed mission and resumes from its elapsed time', async ({ page }) => {
  ...
  await expect(page.locator('#running-lap-indicator')).toContainText('MISSION 2 / 2', { timeout: 3000 });
  await expect(page.locator('#undo-last-lap-button')).not.toHaveClass(hiddenClass);
  await expect(page.locator('#lap-records-list')).toContainText('1등');

  await page.locator('#undo-last-lap-button').click();

  await expect(page.locator('#running-lap-indicator')).toContainText('MISSION 1 / 2');
