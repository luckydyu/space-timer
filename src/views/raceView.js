(() => {
  ...
  function updateMainActionButton() {
    ...
    if (state.appState === 'FOCUS') {
      ...
      btn.className = baseClass + 'bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 hover:to-amber-400 text-slate-950 border-amber-800 animate-pulse';
      btn.innerHTML = `
        <div class="absolute inset-0 bg-white/10 transform -skew-x-12 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
        🏆 기록 확인 완료 · 완주 확정
      `;
  ...
  function updateUndoButton() {
    ...
    const canUndo = ['LAP_WAITING', 'FINAL_REVIEW'].includes(state.appState) && state.lapRecords.length > 0;
  ...
  function updateLapDashboard() {
    ...
    for (let i = 0; i < state.targetLaps; i++) {
      ...
      list.insertAdjacentHTML('beforeend', `
        <div class="flex items-center justify-between p-3 rounded-xl border-2 ${isRecorded ? 'border-emerald-500 bg-emerald-950/30' : isCurrent ? 'border-indigo-500 bg-indigo-500/10 animate-pulse' : 'border-slate-700 bg-slate-900/40'} transition-all text-base md:text-lg">
          ...
          <div class="flex items-center gap-2">
            ${isRecorded ? `<span class="game-font text-sm md:text-base px-2 py-1 rounded-lg bg-indigo-900/80 text-indigo-100 border border-indigo-500">${rank}등</span>` : ''}
            ...
            ${isRecorded ? `<button type="button" data-delete-lap-index="${i}" class="game-font text-sm md:text-base px-2 py-1 rounded-lg bg-slate-800 hover:bg-rose-700 text-slate-300 hover:text-white border border-slate-600 hover:border-rose-400 transition-colors" aria-label="${i + 1}번째 기록 삭제">삭제</button>` : ''}
          </div>
        </div>
      `);
    }
